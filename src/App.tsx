...
/**
 * Заливает МЕЛКИЕ полностью замкнутые «прожоги» альфы: модель пробивает
 * внутри объекта пятна пониженной прозрачности (alpha<200) — от сквозных
 * дыр до полупрозрачных подпалин с каймой (на тёмном фоне выглядят
 * чёрными точками/пунктиром). Дефект = область alpha<200, НЕ достижимая
 * от границ канваса, площадью ≤ max(64px, 1.5% непрозрачного тела).
 * Настоящие отверстия (бублик, дужка) и стеклянные интерьеры больше
 * порога — не трогаются; антиалиасинг силуэта касается внешнего фона
 * (достижим) — не трогается. RGB — из оригинала листа.
 */
